...
No longer uses Sacred. Instead, uses argparse and wandb for logging.
"""
...
def main():
    ...
    accuracies = list()
    losses = list()
    losses_subtasks = dict()
    accuracies_subtasks = dict()
    for i in range(args.n_tasks):
        losses_subtasks[i] = list()
        ...
        if step % args.log_freq == 0:
            with torch.no_grad():
                ...
                for i in range(args.n_tasks):
                    ...
                    y_i_pred = mlp(x_i)
                    losses_subtasks[i].append(loss_fn(y_i_pred, y_i).item())
                    labels_i_pred = torch.argmax(y_i_pred, dim=1)
                ...
                for i in range(args.n_tasks):
                    wandb_metrics[f'accuracy_subtask_{i}'] = accuracies_subtasks[i][-1]
                    wandb_metrics[f'loss_subtask_{i}'] = losses_subtasks[i][-1]
                if use_wandb:
    ...
    results = {
        ...
        'losses': losses,
        'losses_subtasks': losses_subtasks,
